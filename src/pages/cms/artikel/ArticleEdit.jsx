...
const ArticleEdit = () => {
  ...
  const [article, setArticle] = useState(null); // Add missing article state
  ...
  // Fetch article data and tags
  useEffect(() => {
    // Fetch article tags first
    getArticleTags((tags) => {
      setArticleTags(tags);
    });

    if (id) {
      // Since we don't have getArticleById, we'll get all articles and find the one with matching ID
      getArticles((articles) => {
        ...
        if (foundArticle) {
          ...
          setFormArticle({
            judul: foundArticle.judul || "",
            penulis: foundArticle.penulis || "",
            tags: foundArticle.tags || foundArticle.Tags || [],
            deskripsi: foundArticle.deskripsi || "",
            tanggal: foundArticle.tanggal || "",
            gambar: foundArticle.gambar,
            keteranganGambar: foundArticle.keteranganGambar || "",
            tautanGambar: foundArticle.tautanGambar || "",
            altGambar: foundArticle.altGambar || "",
            subArtikels: foundArticle.subArtikels || [],
          });

          // Initialize sub-editor refs and contents
          if (foundArticle.subArtikels && foundArticle.subArtikels.length > 0) {
            const refs = foundArticle.subArtikels.map(() => React.createRef());
            const contents = foundArticle.subArtikels.map(
              (sub) => sub.deskripsi || ""
            );
            setSubEditorRefs(refs);
            setSubEditorContents(contents);
          }

          // Load main image data if exists
          if (foundArticle.gambar) {
            if (typeof foundArticle.gambar === "string") {
              // If gambar is just a filename string
              setMainImagePreview(
                `${import.meta.env.VITE_API_URL}/artikel/image/${
                  foundArticle.gambar
                }`
              );
              getImageArticle(foundArticle.gambar, (imageData) => {
                setMainImageData(imageData);
              });
            } else if (foundArticle.gambar.nama) {
              // If gambar is an object with nama property
              setMainImagePreview(
                `${import.meta.env.VITE_API_URL}/artikel/image/${
                  foundArticle.gambar.nama
                }`
              );
              setMainImageData(foundArticle.gambar);
              setFormArticle((prev) => ({
                ...prev,
                keteranganGambar: foundArticle.gambar.keterangan || "",
                tautanGambar: foundArticle.gambar.tautan || "",
                altGambar: foundArticle.gambar.alt || "",
              }));
            }
          }

          // Load sub-article images data if exist
          if (foundArticle.subArtikels) {
            const previews = {};
            const imageData = {};
            foundArticle.subArtikels.forEach((sub, index) => {
              if (sub.gambar && sub.gambar.nama) {
                previews[index] = `${
                  import.meta.env.VITE_API_URL
                }/artikel/imageSubArtikel/${sub.gambar.nama}`;
                imageData[index] = sub.gambar;
                // Pre-populate sub-article image metadata
                const updatedSubArtikels = [...foundArticle.subArtikels];
                updatedSubArtikels[index] = {
                  ...updatedSubArtikels[index],
                  keteranganGambar: sub.gambar.keterangan || "",
                  tautanGambar: sub.gambar.tautan || "",
                  altGambar: sub.gambar.alt || "",
                };
                setFormArticle((prev) => ({
                  ...prev,
  ...
  const removeSubArtikel = (index) => {
    ...
    // Jika semua sub artikels dihapus, kosongkan previews dan data
    if (updatedSubArtikels.length === 0) {
      setSubImagePreviews({});
      setSubImageData({});
      setSubEditorRefs([]);
      setSubEditorContents([]);
    } else {
  ...
  const handleDeleteSubImage = (index) => {
    ...
    updatedSubArtikels[index].keteranganGambar = "";
    updatedSubArtikels[index].tautanGambar = "";
    updatedSubArtikels[index].altGambar = "";
    setFormArticle((prev) => ({
      ...prev,
      subArtikels: updatedSubArtikels,
    }));
  ...
  const handleSubmit = (e) => {
    ...
    // Append sub artikels only if not empty
    if (formArticle.subArtikels.length > 0) {
    ...
    // Jika kosong, pastikan subArtikels dikirim sebagai array kosong
    else {
      form.append("subArtikels", JSON.stringify([]));
    }
  ...
  // Tambahkan style global untuk menghilangkan margin pada <p> di editor
  useEffect(() => {
    // Inject CSS untuk menghilangkan margin pada <p> di JoditEditor
          ...
          <div className="text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-600 mb-6">
                Artikel telah berhasil diperbarui. Mohon refresh halaman untuk
                      ...
                      <div className="mt-2">
                        ...
                        {mainImageData && (
                          <p className="text-sm text-gray-600 mt-1">
                            Current: {mainImageData.nama || "Existing image"}
                          </p>
                        )}
                      </div>
                    )}
                      ...
                      {subImagePreviews[currentSubIndex] && (
                        <div className="mt-2">
                          <img
                            ...
                            className="w-32 h-24 object-cover rounded-lg"
                          />
                          {subImageData[currentSubIndex] && (
                            <p className="text-sm text-gray-600 mt-1">
                              Current:{" "}
                              {subImageData[currentSubIndex].nama ||
                                "Existing image"}
                            </p>
                          )}
                        </div>
