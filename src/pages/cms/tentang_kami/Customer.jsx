...
const Customer = () => {
    ...
    const validateForm = () => {
        let newErrors = {};
        if (!formData.image && modalType === "add_customer") {
            newErrors.image = "Gambar customer wajib diunggah";
        }
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            if (validTypes.includes(file.type)) {
                ...
                setErrors((prevErrors) => ({
                    ...prevErrors,
                    image: "Gambar customer harus berupa SVG, PNG, JPG, atau JPEG",
                }));
    ...
    const handleUpdateCustomer = () => {
        if (validateForm()) {
            ...
            updateCustomer(selectedCustomer.id, form, (updateData) => {
                ...
                closeModal();
                showToast("Customer berhasil diubah");
            });
        }
    };
    ...
    const openModal = (type, customer = null) => {
        setModalType(type);
        setIsModalOpen(true);
        if (type === "update_customer" && customer) {
            ...
            setPreviewImage(
                `${import.meta.env.VITE_API_URL}/customer/image/${
                    customer.image
                }`,
            );
        } else if (type === "delete_customer" && customer) {
            setSelectedCustomer(customer);
        }
    };
    ...
    return (
        <AdminLayout pageTitle={"Tentang Kami / Customer"}>
                ...
                <div className="bg-[#F5F5F7] w-full rounded-2xl shadow-md py-4 px-6">
                    <div className="flex mb-6 justify-between">
                        ...
                        <Modal
                            open={isModalOpen}
                            onClose={closeModal}
                            size={modalType === "delete_customer" ? "sm" : ""}
                        >
                            {(modalType === "add_customer" ||
                                modalType === "update_customer") && (
                                <>
                                    <Modal.Header
                                        title={
                                            modalType === "add_customer"
